...
public interface PropostaRepository  extends JpaRepository<Proposta, Long> {
	...
	@Query("select p from Proposta p where p.instituicao != (select i from Instituicao i where i.id =?2) and p.id not in (select proposta from Avaliacao a where (a.avaliador =?1 and a.subcriterio in (select id from SubcriterioAvaliacao s where s.perfilAcesso = (select perfilAcesso from Usuario u where u.id =?1))))")
